(function initDominiumMonitor(root, factory) {
  ...
  const BUCKET_CITY_PREFIXES = Object.freeze({
    FTZ: "FORTALEZA",
    NTL: "NATAL",
    PWM: "PARNAMIRIM",
    JCR: "RECIFE",
  ...
  function cityFromBucket(value) {
    const bucket = normalized(value);
    const prefix = Object.keys(BUCKET_CITY_PREFIXES).find((key) => bucket.startsWith(key));
    return prefix ? BUCKET_CITY_PREFIXES[prefix] : "";
  }
  ...
  function normalizeOrder(order, selectedDate) {
    ...
    const operationalClosedRaw = first(order, ["operational_closed", "history_closed", "baixa_historico"]);
    const operationalClosed = [true, 1, "1", "true", "sim", "yes"].includes(
      typeof operationalClosedRaw === "string" ? operationalClosedRaw.toLowerCase() : operationalClosedRaw,
    );
    const operationalClosedAtRaw = text(first(order, ["operational_closed_at", "history_closed_at", "baixa_historico_em"]));
    const operationalClosedAt = parseDate(operationalClosedAtRaw, date);
    // Task status can be an opaque business code (for example "E") while
    // activity_status carries the OFS lifecycle (started, enroute, complete...).
    // A recognized non-pending lifecycle must not be downgraded to "pending"
    // just because the task code is not human-readable.
    const activityLifecycleKind = statusKind(activityStatus);
    const activityLifecycleAuthoritative = ["field", "completed", "canceled", "suspended"].includes(activityLifecycleKind);
    const rawOrderStatus = isUnscheduled
      ? "CANCELADA / NAO AGENDADA"
      : operationalClosed ? "EXECUTADA"
        : (activityLifecycleAuthoritative ? activityStatus : taskStatus || activityStatus);
    const detailState = text(first(order, ["detail_state", "detail_status"])).toLowerCase();
    const statusNeedsValidation = !operationalClosed && Boolean(detailState && detailState !== "complete")
    ...
    return {
      ...
      bucket,
      sourceFile,
      city: text(first(order, ["city", "cidade"])) || cityFromBucket(bucket) || "NAO INFORMADA",
      ...
      operationalClosed,
      operationalClosedAtRaw,
      operationalClosedAt,
      operationalCloseSource: text(first(order, ["operational_close_source", "history_close_source"])),
  ...
  function tec1Category(order) {
    const explicit = normalized(first(order?.source || {}, [
      "tec1_category", "business_type", "order_category", "service_category", "tipo_negocio",
    ]));
    return /ADESAO|ADHESION/.test(explicit) ? "adhesion" : "service";
  }

  function tec1GraceMinutes(order) {
    // Regra operacional Claro: Serviço perde TEC1 ao passar de +59 min;
    // Adesão usa +119 min. Enquanto o discriminador oficial não estiver
    // mapeado, o fallback deliberado é Serviço.
  ...
  function tec1GraceLabel(order) {
    return tec1Category(order) === "adhesion" ? "2H" : "1H";
  }

  function tec1State(order, now) {
    const empty = { key: "unknown", label: "Sem agenda", minutes: null, phase: "unknown",
      windowMinutes: null, graceMinutes: null, countdownDeadline: null };
    if (!order.isScheduled) return empty;
    ...
    const signedMinutes = (target) => {
      ...
      return diff >= 0 ? Math.ceil(diff / 60000) : -Math.ceil(Math.abs(diff) / 60000);
    };
    const windowDiff = windowDeadline.getTime() - now.getTime();
    const deadlineDiff = deadline.getTime() - now.getTime();
    const windowMinutes = signedMinutes(windowDeadline);
    const graceMinutes = signedMinutes(deadline);
    if (deadlineDiff < 0) {
      return { key: "late", label: `TEC1 estourado há ${Math.abs(graceMinutes)} min`, minutes: graceMinutes,
        phase: "late", windowMinutes, graceMinutes, countdownDeadline: deadline };
    }
    if (windowDiff <= 0) {
      return { key: "risk", label: `Janela ferida · prazo de ${tec1GraceLabel(order)} iniciado`, minutes: graceMinutes,
        phase: "grace", windowMinutes, graceMinutes, countdownDeadline: deadline };
    }
    return { key: windowMinutes <= 60 ? "risk" : "safe",
      label: windowMinutes <= 60 ? `${windowMinutes} min para fechar janela` : "Janela no prazo",
      minutes: windowMinutes, phase: "window", windowMinutes, graceMinutes,
      countdownDeadline: windowDeadline };
  }
  ...
  function buildTec1ContractAlerts(rows) {
    ...
    return [...groups.entries()].map(([contract, tasks]) => {
      const ordered = tasks.filter((row) => {
        ...
        return text(row?.tec1_phase) === "window" && Number.isFinite(minutes) && minutes > 0 && minutes <= 30
          && !["completed", "canceled", "suspended"].includes(text(row?.status_kind).toLowerCase());
      }).sort((a, b) => Number(a.tec1_minutes) - Number(b.tec1_minutes));
      if (!ordered.length) return null;
      const focus = ordered[0];
      const minutes = Number(focus.tec1_minutes);
      const threshold = minutes <= 15 ? 15 : 30;
      const technicians = [...new Set(tasks.map((row) => text(row.technician)).filter((value) => value && value !== "-"))];
      const activeTasks = tasks.filter((row) => !["completed", "canceled", "suspended"].includes(text(row?.status_kind).toLowerCase()));
      const taskIds = new Set(activeTasks.map((row) => text(row.os)).filter(Boolean));
      const deadline = text(focus.window_deadline || focus.tec1_countdown_deadline);
      return { key: `window:${contract}:${deadline}:${threshold}`, kind: "window-risk", contract, threshold, minutes,
        deadline, label: `Janela fecha em ${minutes} min`, technician: technicians.join(" e ") || "Nao informado",
        technicians, task_count: taskIds.size || activeTasks.length,
        window_start: text(focus.window_start) || "-", window_end: text(focus.window_end) || "-" };
  ...
  function buildUrgentCloseAlerts(rows, value = new Date()) {
    ...
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      const contract = text(row?.contract);
      const windowDeadline = new Date(row?.window_deadline || "");
      const deadline = new Date(row?.tec1_deadline || "");
      if (!contract || contract === "-" || Number.isNaN(windowDeadline.getTime()) || Number.isNaN(deadline.getTime())) return;
      if (["completed", "canceled", "suspended"].includes(text(row?.status_kind).toLowerCase())) return;
      if (now.getTime() < windowDeadline.getTime()) return;
      const inGrace = now.getTime() <= deadline.getTime();
      const elapsedWindowMinutes = Math.max(0, Math.floor((now.getTime() - windowDeadline.getTime()) / 60000));
      const remainingMinutes = inGrace ? Math.max(0, Math.ceil((deadline.getTime() - now.getTime()) / 60000)) : 0;
      const lateMinutes = inGrace ? 0 : Math.max(1, Math.ceil((now.getTime() - deadline.getTime()) / 60000));
      const score = inGrace ? 1 : 2;
      const current = groups.get(contract);
      if (!current || score > current.score || lateMinutes > current.lateMinutes) {
        groups.set(contract, { row, score, inGrace, elapsedWindowMinutes, remainingMinutes, lateMinutes });
      }
    });
    return [...groups.entries()].map(([contract, entry]) => {
      const { row, inGrace, elapsedWindowMinutes, remainingMinutes, lateMinutes } = entry;
      const graceLabel = text(row.tec1_grace_label) || (Number(row.tec1_grace_minutes) > 60 ? "2H" : "1H");
      const phase = inGrace ? "window-breached" : "tec1-late";
      const kind = inGrace ? "window-breached" : "urgent-late";
      const label = inGrace ? `Janela ferida · prazo de ${graceLabel} iniciado · ${remainingMinutes} min restantes`
        : `TEC1 estourado há ${lateMinutes} min`;
      return {
        key: `${phase}:${contract}:${text(row.window_deadline)}:${text(row.tec1_deadline)}`, kind, phase, contract,
        os: text(row.os) || "-", date: text(row.date) || "-", technician: text(row.technician) || "Nao informado",
        technician_login: text(row.technician_login) || "-", bucket: text(row.bucket) || "-", status: text(row.status) || "-",
        window_start: text(row.window_start) || "-", window_end: text(row.window_end) || "-",
        actual_start: text(row.actual_start) || "-", actual_end: text(row.actual_end) || "-",
        deadline: text(row.tec1_deadline), elapsed_window_minutes: elapsedWindowMinutes,
        remaining_minutes: remainingMinutes, late_minutes: lateMinutes, label,
        message: speechPronunciationText(inGrace
          ? `Atenção. Janela ferida no contrato ${spokenDigits(contract)}. Prazo de ${graceLabel} iniciado. `
            + `Técnico ${text(row.technician) || "nao informado"}. Restam ${remainingMinutes} minutos para o téqui um.`
          : `Alerta urgente. Téqui um estourado no contrato ${spokenDigits(contract)}. `
            + `Técnico ${text(row.technician) || "nao informado"}. Atraso de ${lateMinutes} minutos.`),
      };
    }).sort((a, b) => (a.kind === "urgent-late" ? -1 : 1) - (b.kind === "urgent-late" ? -1 : 1)
      || b.late_minutes - a.late_minutes || a.remaining_minutes - b.remaining_minutes);
  }
  ...
  function buildTec1VoiceMessage(alert) {
    ...
    return speechPronunciationText(
      `Atenção. Faltam ${alert?.threshold} minutos para fechar a janela do contrato ${spokenDigits(alert?.contract)}. `
      + `Avise o técnico ${alert?.technician || "não informado"} sobre possível téqui um.${windowDetail}${taskDetail}`,
    );
  }
  ...
  function buildTvVoiceMessage(rows, value = new Date()) {
    ...
    return (Array.isArray(rows) ? rows : []).slice(0, 2).map((row, index) => {
      ...
      const technician = row?.technician || "não informado";
      const windowDeadline = new Date(row?.window_deadline || "");
      const finalDeadline = new Date(row?.tec1_deadline || "");
      if (row?.deadline_basis === "official_window" && !Number.isNaN(windowDeadline.getTime()) && !Number.isNaN(finalDeadline.getTime())) {
        const windowDiff = windowDeadline.getTime() - now.getTime();
        const finalDiff = finalDeadline.getTime() - now.getTime();
        if (windowDiff > 0) {
          const minutes = Math.max(1, Math.ceil(windowDiff / 60000));
          return `Prioridade ${index + 1}. Faltam ${minutes} minuto${minutes === 1 ? "" : "s"} para fechar a janela. `
            + `Técnico ${technician}. Contrato ${spokenDigits(contractNumber)}. Avise o técnico sobre possível TEC1.`;
        }
        if (finalDiff >= 0) {
          const minutes = Math.max(0, Math.ceil(finalDiff / 60000));
          const graceLabel = text(row?.tec1_grace_label) || (Number(row?.tec1_grace_minutes) > 60 ? "2 horas" : "1 hora");
          return `Prioridade ${index + 1}. Janela ferida. Prazo de ${graceLabel} iniciado. `
            + `Faltam ${minutes} minuto${minutes === 1 ? "" : "s"} para o TEC1. Técnico ${technician}. Contrato ${spokenDigits(contractNumber)}.`;
        }
        const lateMinutes = Math.max(1, Math.ceil(Math.abs(finalDiff) / 60000));
        return `Prioridade ${index + 1}. TEC1 estourado há ${lateMinutes} minuto${lateMinutes === 1 ? "" : "s"}. `
          + `Baixe imediatamente o contrato ${spokenDigits(contractNumber)}. Técnico ${technician}.`;
      }
      const deadline = new Date(row?.tec1_countdown_deadline || row?.tec1_deadline || "");
      const diff = deadline.getTime() - now.getTime();
      const minutes = Number.isFinite(diff) ? Math.max(1, Math.ceil(Math.abs(diff) / 60000)) : null;
      return `Prioridade ${index + 1}. Técnico ${technician}. Contrato ${spokenDigits(contractNumber)}. `
        + `${minutes === null ? "tempo não informado" : diff < 0 ? `atrasada ${minutes} minutos` : `faltam ${minutes} minutos`}.`;
    }).join(" ");
  }
  ...
  function orderRow(order, now) {
    ...
    return {
      ...
      tec1: tec1.label,
      tec1_kind: tec1.key,
      tec1_phase: tec1.phase,
      tec1_minutes: tec1.minutes,
      tec1_window_minutes: tec1.windowMinutes,
      tec1_grace_remaining_minutes: tec1.graceMinutes,
      window_deadline: windowDeadline ? windowDeadline.toISOString() : "",
      tec1_countdown_deadline: tec1.countdownDeadline ? tec1.countdownDeadline.toISOString() : "",
      tec1_deadline: deadline ? deadline.toISOString() : "",
      tec1_category: tec1Category(order),
      tec1_grace_minutes: tec1GraceMinutes(order),
      tec1_grace_label: tec1GraceLabel(order),
      operational_closed: order.operationalClosed,
      operational_closed_at: order.operationalClosedAtRaw || "",
      operational_close_source: order.operationalCloseSource || "",
    ...
    const routePriorities = activeRows
      ...
      .map((row) => {
        ...
        return {
          ...
          tec1_countdown_deadline: row.route_deadline,
