...
export function monitorTvCountdown(deadlineOrRow, now = new Date()) {
  const row = deadlineOrRow && typeof deadlineOrRow === 'object' ? deadlineOrRow : null;
  const deadline = row ? (row.tec1_countdown_deadline || row.tec1_deadline) : deadlineOrRow;
  if (!deadline) return { text: '--:--:--', kind: 'unknown', urgent: false, lateMinutes: 0, phase: row?.tec1_phase || 'unknown' };
  ...
  return {
    ...
    kind: row?.tec1_kind || (diff < 0 ? 'late' : diff <= 3600000 ? 'risk' : 'safe'),
    urgent: row ? ['grace', 'late'].includes(phase) : lateMinutes >= 45,
    lateMinutes,
    phase,
  };
}
...
const TV_KPI_LABELS = Object.freeze({
  total: 'TOTAL DE OS',
  field: 'EM CAMPO',
  completed: 'CONCLUÍDAS',
  pending: 'PENDENTES',
  risk: 'JANELA / TEC1 EM ATENÇÃO',
  late: 'TEC1 ESTOURADO',
  routes: 'ALERTAS DE ROTA',
});

export function tvKpiRows(model, key) {
  const rows = model?.views?.monitor?.rows || [];
  if (key === 'total') return rows;
  if (key === 'field') return rows.filter((row) => row.status_kind === 'field');
  if (key === 'completed') return rows.filter((row) => row.status_kind === 'completed');
  if (key === 'pending') return rows.filter((row) => row.status_kind === 'pending');
  if (key === 'risk') return rows.filter((row) => row.tec1_kind === 'risk');
  if (key === 'late') return rows.filter((row) => row.tec1_kind === 'late');
  if (key === 'routes') {
    const alerted = new Set((model?.views?.routes?.console?.alerts || []).map((item) => String(item.os || '')));
    return rows.filter((row) => alerted.has(String(row.os || '')));
  }
  return [];
}

let tvDrilldownState = null;

function tvRowKey(row) {
  return `${row?.activityId || row?.activity_id || ''}:${row?.os || ''}`;
}

function tvRowSearchText(row) {
  return normalize([
    row?.os, row?.contract, row?.service, row?.technician, row?.technician_login,
    row?.bucket, row?.status, row?.city, row?.activityId, row?.close_code,
  ].join(' '));
}

function tvDrilldownRows(model, state) {
  const rows = model?.views?.monitor?.rows || [];
  if (!state) return [];
  if (state.type === 'kpi') return tvKpiRows(model, state.key);
  if (state.type === 'technician') {
    const login = normalize(state.login);
    const name = normalize(state.name);
    return rows.filter((row) => (login && normalize(row.technician_login) === login)
      || (name && normalize(row.technician) === name));
  }
  return [];
}

function tvDrilldownTitle(state) {
  if (state?.type === 'technician') return state.name || state.login || 'TÉCNICO';
  return TV_KPI_LABELS[state?.key] || 'DETALHES DA OPERAÇÃO';
}

function tvDrilldownSubtitle(state, count) {
  if (state?.type === 'technician') return `${count} OS vinculada${count === 1 ? '' : 's'} ao técnico na leitura atual`;
  return `${count} OS na leitura atual · clique em uma linha para abrir os detalhes`;
}

function tvRenderListRow(row, index) {
  const tec1 = row?.tec1 && row.tec1 !== '-' ? row.tec1 : row?.status || '-';
  const statusClass = escapeHtml(row?.status_kind || 'pending');
  return `<button type="button" class="monitor-tv-drill-row" data-tv-drill-row="${index}">
    <span class="monitor-tv-drill-row-main"><strong>OS ${escapeHtml(row?.os || '-')}</strong><small>${escapeHtml(row?.service || 'Atividade')}</small></span>
    <span><small>Contrato</small><strong>${escapeHtml(row?.contract || '-')}</strong></span>
    <span><small>Técnico</small><strong>${escapeHtml(row?.technician || '-')}</strong></span>
    <span><small>Bucket</small><strong>${escapeHtml(row?.bucket || '-')}</strong></span>
    <span class="monitor-tv-drill-row-status ${statusClass}"><small>Status</small><strong>${escapeHtml(row?.status || '-')}</strong></span>
    <span><small>TEC1</small><strong>${escapeHtml(tec1)}</strong></span>
    <i data-lucide="chevron-right"></i>
  </button>`;
}

function tvFormatClosedAt(value) {
  if (!value) return '-';
  return formatPtBrDateTime(String(value));
}

function tvRenderOrderDetail(row) {
  const facts = [
    ['OS', row?.os], ['Contrato', row?.contract], ['Atividade', row?.activity_id],
    ['Serviço', row?.service], ['Status', row?.status], ['Técnico', row?.technician],
    ['Login TOA', row?.technician_login], ['Bucket', row?.bucket], ['Cidade', row?.city],
    ['Agenda', formatPtBrSchedule(row?.schedule)], ['Janela', `${row?.window_start || '-'} — ${row?.window_end || '-'}`],
    ['Fase JANELA/TEC1', row?.tec1_phase], ['Fim da janela', tvFormatClosedAt(row?.window_deadline)],
    ['Prazo final TEC1', tvFormatClosedAt(row?.tec1_deadline)], ['Código de baixa', row?.close_code],
    ['Baixa operacional', tvFormatClosedAt(row?.operational_closed_at)],
    ['Origem do encerramento', row?.operational_close_source], ['TEC1', row?.tec1],
  ];
  return `<div class="monitor-tv-order-detail-grid">${facts.map(([label, value]) => `<span><small>${label}</small><strong>${escapeHtml(value || '-')}</strong></span>`).join('')}</div>`;
}

function tvFindDetailRow(model, state) {
  const rows = model?.views?.monitor?.rows || [];
  return rows.find((row) => tvRowKey(row) === state?.rowKey)
    || rows.find((row) => String(row?.os || '') === String(state?.os || ''))
    || null;
}

function tvOpenRowInMonitor(store, row) {
  const needle = String(row?.os || row?.contract || '').trim();
  tvDrilldownState = null;
  exitTv();
  store.set({ module: 'monitor', view: 'monitor', search: needle, bucket: 'all', status: 'all' });
  switchModule(store, 'monitor');
  const input = document.querySelector('#monitorSearch');
  if (input) input.value = needle;
  const status = document.querySelector('#monitorStatus');
  if (status) status.value = 'all';
}

function tvBindKeyboardAction(element, handler) {
  if (!element) return;
...
function renderTvDrilldown(tvRoot, store, alertService, model) {
  tvRoot.querySelector('.monitor-tv-drilldown')?.remove();
  if (!tvDrilldownState) return;
  const overlay = document.createElement('section');
  overlay.className = 'monitor-tv-drilldown';
  overlay.setAttribute('role', 'dialog');
  overlay.setAttribute('aria-modal', 'true');

  if (tvDrilldownState.type === 'detail') {
    const row = tvFindDetailRow(model, tvDrilldownState);
    if (!row) { tvDrilldownState = null; return; }
    overlay.setAttribute('aria-label', `Detalhes da OS ${row.os || ''}`);
    overlay.innerHTML = `<div class="monitor-tv-drill-panel detail">
      <header><div><span>DETALHE OPERACIONAL</span><h2>OS ${escapeHtml(row.os || '-')}</h2><small>${escapeHtml(row.service || 'Atividade')}</small></div><button type="button" data-tv-drill-close aria-label="Fechar"><i data-lucide="x"></i></button></header>
      ${tvRenderOrderDetail(row)}
      <footer><button type="button" class="secondary" data-tv-drill-back><i data-lucide="arrow-left"></i>Voltar</button><button type="button" class="secondary" data-tv-copy-os><i data-lucide="copy"></i>Copiar OS</button><button type="button" class="primary" data-tv-open-monitor><i data-lucide="external-link"></i>Abrir no monitor</button></footer>
    </div>`;
    tvRoot.append(overlay);
    overlay.querySelector('[data-tv-drill-close]')?.addEventListener('click', () => { tvDrilldownState = null; overlay.remove(); });
    overlay.querySelector('[data-tv-drill-back]')?.addEventListener('click', () => { tvDrilldownState = tvDrilldownState.returnState || null; renderTv(store, alertService); });
    overlay.querySelector('[data-tv-copy-os]')?.addEventListener('click', async () => {
      try { await navigator.clipboard.writeText(String(row.os || '')); toast(`OS ${row.os} copiada.`); } catch { toast('Não foi possível copiar a OS.', 'error'); }
    });
    overlay.querySelector('[data-tv-open-monitor]')?.addEventListener('click', () => tvOpenRowInMonitor(store, row));
    overlay.addEventListener('click', (event) => { if (event.target === overlay) { tvDrilldownState = null; overlay.remove(); } });
    window.lucide?.createIcons();
    return;
  }

  const allRows = tvDrilldownRows(model, tvDrilldownState);
  const term = normalize(tvDrilldownState.search || '');
  const filtered = term ? allRows.filter((row) => tvRowSearchText(row).includes(term)) : allRows;
  const visible = filtered.slice(0, 120);
  const title = tvDrilldownTitle(tvDrilldownState);
  const subtitle = tvDrilldownSubtitle(tvDrilldownState, filtered.length);
  overlay.setAttribute('aria-label', title);
  overlay.innerHTML = `<div class="monitor-tv-drill-panel">
    <header><div><span>DRILL-DOWN</span><h2>${escapeHtml(title)}</h2><small>${escapeHtml(subtitle)}</small></div><button type="button" data-tv-drill-close aria-label="Fechar"><i data-lucide="x"></i></button></header>
    <div class="monitor-tv-drill-toolbar"><label><i data-lucide="search"></i><input type="search" data-tv-drill-search value="${escapeHtml(tvDrilldownState.search || '')}" placeholder="Buscar OS, contrato, técnico, bucket..."></label><strong>${visible.length}${filtered.length > visible.length ? ` de ${filtered.length}` : ''}</strong></div>
    <div class="monitor-tv-drill-list">${visible.length ? visible.map(tvRenderListRow).join('') : '<div class="monitor-tv-drill-empty">Nenhuma OS encontrada neste filtro.</div>'}</div>
    <footer><span>Os dados continuam atualizando em segundo plano.</span><button type="button" class="secondary" data-tv-drill-close>Fechar</button></footer>
  </div>`;
  tvRoot.append(overlay);
  overlay.querySelectorAll('[data-tv-drill-close]').forEach((button) => button.addEventListener('click', () => { tvDrilldownState = null; overlay.remove(); }));
  const input = overlay.querySelector('[data-tv-drill-search]');
  input?.addEventListener('input', (event) => {
    tvDrilldownState = { ...tvDrilldownState, search: event.target.value };
    renderTvDrilldown(tvRoot, store, alertService, buildModel(store.get()));
    tvRoot.querySelector('[data-tv-drill-search]')?.focus();
  });
  overlay.querySelectorAll('[data-tv-drill-row]').forEach((button) => {
    const index = Number(button.dataset.tvDrillRow || -1);
    button.addEventListener('click', () => {
      const row = visible[index];
      if (!row) return;
      const returnState = { ...tvDrilldownState };
      tvDrilldownState = { type: 'detail', rowKey: tvRowKey(row), os: row.os, returnState };
...
function renderTvTechnicianCards(items) {
  return items.length ? items.map((item, index) => `<article class="monitor-tv-clickable" data-tv-technician-index="${index}" role="button" tabindex="0" aria-label="Abrir atividades de ${escapeHtml(item.technician)}"><span class="avatar">${escapeHtml(item.technician.split(/\s+/).slice(0, 2).map((part) => part[0] || '').join(''))}</span><div><strong>${escapeHtml(item.technician)}</strong><small>OS ${escapeHtml(item.os)} · ${escapeHtml(item.service)}</small><b>${escapeHtml(item.state_label)} · ${escapeHtml(item.bucket)}</b></div><span class="monitor-tv-card-open"><i data-lucide="chevron-right"></i></span></article>`).join('') : '<div class="monitor-tv-empty">Nenhum técnico iniciado na leitura atual.</div>';
}

function bindTvInteractions(tvRoot, store, alertService, model, focusPage, technicianPage) {
  tvRoot.querySelectorAll('[data-tv-kpi]').forEach((card) => tvBindKeyboardAction(card, () => {
    tvDrilldownState = { type: 'kpi', key: card.dataset.tvKpi, search: '' };
    renderTv(store, alertService);
  }));
  tvRoot.querySelectorAll('[data-tv-focus-slot]').forEach((card) => tvBindKeyboardAction(card, () => {
    const row = focusPage.items[Number(card.dataset.tvFocusSlot || 0)];
    if (!row || !row.os || row.os === '-') return;
    tvDrilldownState = { type: 'detail', rowKey: tvRowKey(row), os: row.os, returnState: null };
    renderTv(store, alertService);
  }));
  tvRoot.querySelectorAll('[data-tv-technician-index]').forEach((card) => tvBindKeyboardAction(card, () => {
    const item = technicianPage.items[Number(card.dataset.tvTechnicianIndex || 0)];
    if (!item) return;
  ...
  renderTvDrilldown(tvRoot, store, alertService, model);
}
...
function tvTec1PhasePresentation(focus, countdown) {
  ...
  const phase = focus?.tec1_phase || countdown?.phase || 'window';
  const grace = focus?.tec1_grace_label || (Number(focus?.tec1_grace_minutes) > 60 ? '2H' : '1H');
  if (phase === 'late') return {
    active: true, css: 'tec1-late', title: 'TEC1 ESTOURADO',
    detail: `Prazo final vencido · baixa imediata do contrato ${focus?.contract || '-'}`,
  };
  if (phase === 'grace') return {
    active: true, css: 'window-breached', title: `JANELA FERIDA · PRAZO DE ${grace} INICIADO`,
    detail: `${countdown.text} para o TEC1 · cobre a baixa com o técnico`,
  };
  if (phase === 'window' && focus?.tec1_kind === 'risk') return {
    active: true, css: 'window-warning', title: `JANELA FECHA EM ${countdown.text}`,
    detail: 'Aviso preventivo · fale com o técnico sobre possível TEC1',
  };
  return { active: false, css: 'window-safe', title: 'CONTAGEM ATÉ O FIM DA JANELA', detail: '' };
}

function renderTvFocusCard(focus, slot) {
  const countdown = monitorTvCountdown(focus);
  const phase = tvTec1PhasePresentation(focus, countdown);
  const urgent = ['grace', 'late'].includes(focus?.tec1_phase) && focus.deadline_basis === 'official_window';
  const primaryFacts = [
    ['Contrato', focus.contract], ['Técnico', focus.technician],
    ['Status', focus.status], ['Agenda', formatPtBrSchedule(focus.schedule)],
  ];
  const secondaryFacts = [
    ['OS', focus.os], ['Cidade', focus.city], ['Bucket', focus.bucket],
    ['Janela', `${focus.window_start || '-'} — ${focus.window_end || '-'}`],
  ];
  const phaseIcon = focus?.tec1_phase === 'window' ? 'triangle-alert' : 'siren';
  return `<section class="monitor-tv-focus-card monitor-tv-clickable ${countdown.kind} phase-${escapeHtml(focus?.tec1_phase || 'unknown')}${urgent ? ' urgent-close' : ''}" data-tv-focus-slot="${slot}" data-tv-phase="${escapeHtml(focus?.tec1_phase || 'unknown')}" role="button" tabindex="0" aria-label="Abrir detalhes da OS ${escapeHtml(focus.os || '-')}">
    <div class="monitor-tv-focus-card-head"><span>PRIORIDADE ${slot + 1}</span><span class="monitor-tv-status ${escapeHtml(focus.tec1_kind || 'unknown')}">${escapeHtml(focus.tec1 || focus.status || 'Sem agenda')}</span></div>
    <div class="monitor-tv-countdown-caption">${escapeHtml(focus?.tec1_phase === 'grace' ? 'TEMPO ATÉ ESTOURAR O TEC1' : focus?.tec1_phase === 'late' ? 'TEMPO APÓS ESTOURAR O TEC1' : 'TEMPO ATÉ FECHAR A JANELA')}</div>
    <strong class="monitor-tv-tec1-value ${countdown.kind}" data-tv-countdown>${escapeHtml(countdown.text)}</strong>
    <h1>${escapeHtml(focus.service || 'Atividade')}</h1>
    <div class="monitor-tv-focus-details"><div class="monitor-tv-focus-grid monitor-tv-focus-primary">${renderTvFocusFacts(primaryFacts)}</div><div class="monitor-tv-focus-grid monitor-tv-focus-secondary">${renderTvFocusFacts(secondaryFacts)}</div></div>
    <div class="monitor-tv-urgent ${phase.css}${phase.active ? ' active' : ''}" data-tv-urgent><span class="monitor-tv-siren"><i data-lucide="${phaseIcon}"></i></span><div><strong>${escapeHtml(phase.title)}</strong><small>${escapeHtml(phase.detail)}</small></div></div>
  </section>`;
}
...
function renderTv(store, alertService) {
  ...
  const focusTitle = routeEstimate ? 'PRIORIDADES DA ROTA' : 'JANELA / TEC1 PRIORITÁRIOS';
  const focusSubtitle = routeEstimate
    ? 'Estimativa pelo fim da agenda · janela oficial pendente'
    : '1º aviso: fim da janela · 2º aviso: prazo TEC1 após janela ferida';
  const focusBasisText = routeEstimate
    ? 'Estimativa operacional pelo fim da agenda do TOA; não representa TEC1 oficial.'
    : 'Primeiro cronômetro termina no fim da janela; sem baixa, inicia o prazo adicional do TEC1.';
  ...
  bindTvInteractions(tvRoot, store, alertService, model, focusPage, technicianPage);
...
function updateTvLive(store, alertService) {
  ...
  document.querySelectorAll('[data-tv-focus-slot]').forEach((card) => {
    ...
    if (!focusItem) return;
    if (card.dataset.tvPhase !== String(focusItem.tec1_phase || 'unknown')) { phaseChanged = true; return; }
    const countdown = monitorTvCountdown(focusItem, now);
    const presentation = tvTec1PhasePresentation(focusItem, countdown);
    const urgent = ['grace', 'late'].includes(focusItem?.tec1_phase) && focusItem?.deadline_basis === 'official_window';
    const countdownRoot = card.querySelector('[data-tv-countdown]');
    card.className = `monitor-tv-focus-card monitor-tv-clickable ${countdown.kind} phase-${focusItem.tec1_phase || 'unknown'}${urgent ? ' urgent-close' : ''}`;
    if (countdownRoot) { countdownRoot.textContent = countdown.text; countdownRoot.className = `monitor-tv-tec1-value ${countdown.kind}`; }
    const statusRoot = card.querySelector('.monitor-tv-status');
    if (statusRoot) { statusRoot.textContent = focusItem.tec1 || focusItem.status || 'Sem agenda'; statusRoot.className = `monitor-tv-status ${focusItem.tec1_kind || 'unknown'}`; }
    const urgentRoot = card.querySelector('[data-tv-urgent]');
    if (urgentRoot) {
      urgentRoot.className = `monitor-tv-urgent ${presentation.css}${presentation.active ? ' active' : ''}`;
      const title = urgentRoot.querySelector('strong');
      const detail = urgentRoot.querySelector('small');
      if (title) title.textContent = presentation.title;
      if (detail) detail.textContent = presentation.detail;
    }
  });
  if (phaseChanged) { renderTv(store, alertService); return; }
...
export async function createApp(root) {
  ...
  if (datalakeUsesRealtime()) {
    ...
    const recoverRealtimeFeed = async () => {
      if (watchdogBusy || Date.now() - lastRealtimeEventAt < 25000) return;
      watchdogBusy = true;
      try {
        if (await refreshDatalake(store, directory, alertService)) lastRealtimeEventAt = Date.now();
      } finally {
        watchdogBusy = false;
      }
    };
    await subscribeDatalakeFeed(
      (snapshot) => {
        ...
        acceptRealtimeSnapshot(store, directory, alertService, snapshot);
      },
      (error) => {
    ...
    setInterval(recoverRealtimeFeed, 15000);
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') recoverRealtimeFeed();
    });
  } else {
