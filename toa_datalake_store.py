...
def _executed_os_status(value: Any) -> bool:
    status = _ascii(value).replace("_", " ")
    if any(term in status for term in ("nao executad", "not done", "notdone", "cancel")):
        return False
    return any(term in status for term in ("executad", "executed"))


def _history_local_iso(value: Any) -> str:
    raw = _text(value, 40)
...
def _operational_close_from_outcomes(outcomes: list[dict[str, Any]], expected_tasks: int = 0) -> dict[str, Any]:
    relevant = [item for item in outcomes if _text(item.get("os_status")) or _text(item.get("close_code"))]
    required = max(1, int(expected_tasks or 0))
    if len(relevant) < required:
        return {}
    if any(not _executed_os_status(item.get("os_status")) or not _digits(item.get("close_code")) for item in relevant):
        return {}
    closed_at_values = [_history_local_iso(item.get("tabulated_at")) for item in relevant]
    closed_at_values = [value for value in closed_at_values if value]
    return {
        "closed": True,
        "closed_at": max(closed_at_values) if closed_at_values else "",
        "codes": sorted({_digits(item.get("close_code")) for item in relevant if _digits(item.get("close_code"))}),
        "tasks": len(relevant),
        "source": "history_os_executed",
    }
...
class TOADatalakeStore:
    ...
    def detail_queue(self, limit: int = 100) -> dict[str, Any]:
        with self.lock, self._connect() as db:
            rows = [dict(row) for row in db.execute("""SELECT a.activity_id,a.contract,a.bucket,a.scheduled_date,
              ...
              GROUP_CONCAT(o.os_number) os_numbers, GROUP_CONCAT(DISTINCT o.status) order_statuses, GROUP_CONCAT(DISTINCT o.close_code) close_codes FROM activities a LEFT JOIN orders o ON o.activity_id=a.activity_id
    ...
    def feed(self, profile: str = "", date: str = "") -> dict[str, Any]:
        ...
        with self.lock, self._connect() as db:
            ...
            history_by_activity = {row["activity_id"]: [] for row in activities}
            if linked:
                ...
                for row in db.execute(
                    f"SELECT * FROM activity_history WHERE activity_id IN ({marks}) ORDER BY action_timestamp,action_time",
                    list(linked),
                ):
                    item = dict(row)
                    item["changes"] = json.loads(item.pop("changes_json") or "[]")
                    history_by_activity[item["activity_id"]].append(item)
        ...
        for a in activities:
            ...
            if start_m is not None and effective_window and "-" in effective_window:
                ...
                if len(parts) == 2 and ":" in parts[1]:
                    try:
                        ...
                        pass
            history_rows = history_by_activity.get(effective["activity_id"], []) or history_by_activity.get(a["activity_id"], [])
            history_summary = self._history_summary(history_rows) if history_rows else {"os_outcomes": []}
            operational_close = _operational_close_from_outcomes(
                history_summary.get("os_outcomes", []), len(linked.get(a["activity_id"], [])) or 1,
            )
            common = {"profile": effective["profile"], "date": effective["scheduled_date"], "scheduled_date": effective["scheduled_date"],
              ...
              "operational_closed": bool(operational_close.get("closed")),
              "operational_closed_at": operational_close.get("closed_at", ""),
              "operational_close_source": operational_close.get("source", ""),
              "operational_close_codes": operational_close.get("codes", []),
