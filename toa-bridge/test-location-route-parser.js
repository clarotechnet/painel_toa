...
assert.match(source, /function compactMapVisitRows/);
assert.match(source, /compactIndex >= 0/);
assert.match(source, /map-special:/);
assert.match(source, /Number\(value\.g\) === 2/);
assert.match(source, /raw < 1_000_000_000/);
assert.doesNotMatch(source, /const plannedRoute = serviceStops\.map/);
assert.match(source, /replace_service_stops: Boolean\(resource\.replace_visits\)/);
